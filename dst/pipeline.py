...
class Pipeline1F1B:
    def __init__(self, stage: GPTStage, pp: PPContext, micro_batch_size: int, seq_len: int):
        ...
        # p2p buffers must match the activations' dtype (bf16 under mixed
        # precision) and device; the params carry both.
        p0 = next(stage.parameters())
        self.act_dtype, self.act_device = p0.dtype, p0.device
    ...
    def _recv_forward(self) -> Optional[torch.Tensor]:
        if self.pp.is_first:
            return None
        x = torch.empty(self.act_shape, dtype=self.act_dtype, device=self.act_device)
    ...
    def _recv_backward(self) -> Optional[torch.Tensor]:
        if self.pp.is_last:
            return None
        g = torch.empty(self.act_shape, dtype=self.act_dtype, device=self.act_device)
